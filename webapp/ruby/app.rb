  ...
  class App < Sinatra::Base
    ...
    get '/api/train/search' do
      ...
      train_list = if params[:train_class].nil? || params[:train_class].empty?
        db.xquery(
          'SELECT * FROM `train_master` WHERE `date` = ? AND `train_class` IN (?) AND `is_nobori` = ? ORDER BY `date`',
          date.strftime('%Y/%m/%d'),
        ...
        db.xquery(
          'SELECT * FROM `train_master` WHERE `date` = ? AND `train_class` IN (?) AND `is_nobori` = ? AND `train_class` = ? ORDER BY `date`',
          date.strftime('%Y/%m/%d'),
          usable_train_class_list,
          is_nobori,
          params[:train_class],
    ...
    get '/api/train/seats' do
      ...
      train = db.xquery(
        'SELECT * FROM `train_master` WHERE `date` = ? AND `train_class` = ? AND `train_name` = ? ORDER BY `date`',
        date.strftime('%Y/%m/%d'),
        params[:train_class],
        params[:train_name],
      ).first
